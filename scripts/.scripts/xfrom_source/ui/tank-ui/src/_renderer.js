...
function displayLine() {
    ...
    switch(true) {
        case i === 2:
            bootScreen.innerHTML += `TANK Kernel version ${electron.remote.app.getVersion()} boot at ${Date().toString()}; root:xnu-1699.22.73~1/RELEASE_X86_64`;
        case i === 4:
...
async function displayTitleScreen() {
    ...
    bootScreen.innerHTML = "<h1>TANKSON</h1>";
...
async function initUI() {
    ...
    window.term[0].term.writeln("\033[1m"+`Welcome Chase v${electron.remote.app.getVersion()} - Electron v${process.versions.electron}`+"\033[0m");
